...
def load_data_dialog() -> pd.DataFrame:
    ...
    df_dialog['utterance content'] = df_dialog[0].apply(lambda x: x.split(' ', 1)[1].lower())
    ...
    def hard_utterance_classification(self):
        for name, classifier in self.classifiers.items():
            ...
            results = pd.DataFrame({
                'utterance content': self.data['utterance content'].iloc[y_test.index],
                'actual dialog act': y_test,
                'predicted dialog act': y_pred
            })

            # Identify incorrect classifications
            incorrect_classifications = results[results['actual dialog act'] != results['predicted dialog act']]
            print(f"{name}: Incorrectly classified utterances with duplicates:\n", incorrect_classifications[['utterance content', 'actual dialog act', 'predicted dialog act']])

            # Save the incorrect classifications to a CSV file
            incorrect_file_name = f"{name}_incorrect_classifications_with_duplicates.csv"
            incorrect_classifications.to_csv(incorrect_file_name, index=False)
            print(f"Saved incorrectly classified utterances with dupcliates to {incorrect_file_name}")
        ...
        for name, classifier in self.classifiers.items():
            ...
            # Identify incorrect classifications
            incorrect_classifications = results[results['actual dialog act'] != results['predicted dialog act']]
            print(f"{name}: Incorrectly classified utterances without duplicates:\n", incorrect_classifications[['utterance content', 'actual dialog act', 'predicted dialog act']])

            # Save the incorrect classifications to a CSV file
            incorrect_file_name = f"{name}_incorrect_classifications_without_duplicates.csv"
            incorrect_classifications.to_csv(incorrect_file_name, index=False)
            print(f"Saved incorrectly classified utterances without duplicates to {incorrect_file_name}")
